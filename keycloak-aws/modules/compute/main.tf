...
data "aws_iam_policy_document" "keycloak_permissions" {
  ...
  # Statement 4: read the realm file from S3, if one was uploaded there.
  dynamic "statement" {
    for_each = var.realm_s3_bucket != "" ? [1] : []
    ...
    content {
      ...
      resources = ["arn:aws:s3:::${var.realm_s3_bucket}/${var.realm_s3_key}"]
    }
  }
}
